"""
Game Data Module for Monster Hunter
Compatible with Pygame CE 2.5.5 and Python 3.13.7
Contains trainer data, monster stats, and attack information
"""
...
# Monster Stats and Evolution Data
MONSTER_DATA = {
	'Plumette': {
		'stats': {
			'element': 'plant',
...
# Helper functions for Python 3.13.7 type safety
def get_monster_data(monster_name: str) -> dict | None:
	"""Safely retrieve monster data by name"""
	return MONSTER_DATA.get(monster_name)

def get_trainer_data(trainer_id: str) -> dict | None:
	"""Safely retrieve trainer data by ID"""
	return TRAINER_DATA.get(trainer_id)

def get_attack_data(attack_name: str) -> dict | None:
	"""Safely retrieve attack data by name"""
	return ATTACK_DATA.get(attack_name)

def is_valid_monster(monster_name: str) -> bool:
	"""Check if a monster exists in the database"""
	return monster_name in MONSTER_DATA

def is_valid_attack(attack_name: str) -> bool:
	"""Check if an attack exists in the database"""
	return attack_name in ATTACK_DATA

def get_monster_evolution(monster_name: str) -> tuple | None:
	"""Get evolution data for a monster (evolved_name, level)"""
	monster = get_monster_data(monster_name)
	return monster['evolve'] if monster else None